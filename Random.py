...
class randMC:
    ...
    def genNormalMatrix(self, steps, sims):
        self.antithetic = False
        array = self.genNormal(steps*sims//2)        
        
        answer = np.zeros([steps, sims])
        
        for i in xrange(steps):
            answer[i] = np.append(array[(i*sims//2):((i+1)*sims//2)],-array[(i*sims//2):((i+1)*sims//2)])
